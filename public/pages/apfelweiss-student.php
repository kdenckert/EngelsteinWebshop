<section class="aw-praesi">
    <figure class="speaker">
        <img src="public/img/apfelweis-teaser.png" width="900" alt="Apfelweiß Evo"/>
    </figure>
    <section class="intro">
        <p>
            Der Apfelweiß ist ein passiver Nah- und Mittenfeldmonitor, entwickelt für die speziellen Hörbedürfnisse von Tontechnikern im Bereich der Studioproduktion.
            Er zeichnet sich besonders durch einen lineraen Frequenzverlauf, die präzise Wiedergabe von Räumlichkeit und minimierten harmonischen Obertöne aus.
            Der Apfwelweiß Student bietet höchste Klangqualität im optisch robusten Gehäuse. Ähnlich einer Gitarre
            verleihen ihm etwaige Gebrauchsspuren eher den Glanz der Erfahrung als ein abgenutztes Aussehen. </p>
    </section>
    <section class="intrest">
        <h2>Welcher Aspekt interessiert dich?</h2>
        <nav id="order">
            <a href="#philosophy" class="philosophie_link praesi-link"><img src="public/img/philosophy_icon.png"
                                                                            width="130" alt=""/><br/>Philosophie</a>
            <a href="#optics" class="optics_link praesi-link"><img src="public/img/optic_icon.png" width="130"
                                                                   alt=""/><br/>Optik</a>
            <a href="#technic" class="technical_link praesi-link"><img src="public/img/icon-technical.png" width="130"
                                                                       alt=""/><br/>Technik</a>
            <a href="#sound" class="sound_link praesi-link"><img src="public/img/diamond.png" width="130" alt=""/><br/>Klang</a>
        </nav>
    </section>

    <?php include 'inc/student_philosophie.inc.php'; ?>

    <?php include 'inc/student_optik.inc.php'; ?>

    <?php include 'inc/student_technik.inc.php'; ?>

    <?php include 'inc/klang.inc.php'; ?>

    <section class="result">
        <h2 class="light">Gewähltes Produkt</h2>
        <ul>
            <li><span>Menge:</span> Stereo</li>
            <li class="final_style"><span>Stil:</span> <strong>Student</strong></li>
            <li class="final_price"><span>Preis:</span> <strong>3.000,00 €</strong></li>
        </ul>
        <h2><a href="warenkorb&amp;item=AWStudent&style=student" class="addtocart no-customization">In den Warenkorb</a>
        </h2>
    </section>